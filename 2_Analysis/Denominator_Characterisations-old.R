...
BCSP_patients  <- cdm$measurement %>% # note that this is edited from the originial code that joins to the ancestor table as that didnt provide any records
  select(person_id,measurement_concept_id, measurement_date) %>%
...
PSA_patients  <- cdm$measurement %>% # note that this code is different from my original code which joins to ancestor table as that didn't work
  select(person_id,measurement_concept_id, measurement_date) %>%
